...
class pathfind():
    ...
    def __init__(self, map_file, robot_radius, start, goal):
        ...
        self.threshold()
        self.inflate()
        self.distance = cv2.distanceTransform(self.map, cv2.DIST_L2, 3)
        self.dist_weight = 70/(self.distance + 0.00001)
    ...
    def get_neighbors(self,current):
        ...
        if(current[1]+1<self.rows and self.map[current[0],current[1]+1]==255):
            neighbors.append((current[0], current[1]+1))
        if(current[0]+1<self.columns and current[1]+1<self.columns and self.map[current[0]+1,current[1]+1]==255):
    ...
    def astar(self):
        ...
        """
        frontier = {}
        frontier[self.start] = heuristic_weight * math.dist(self.goal, self.start)
        ...
        while frontier:
            ...
            for next in neighbors:
                new_cost = cost_so_far[current] + math.dist(current, next) + self.dist_weight[next]
...
def main():
    ...
    d1 = pathfind(map_file,1,start,goal)
    begin = time.time()
    d1.astar()
    end = time.time()
