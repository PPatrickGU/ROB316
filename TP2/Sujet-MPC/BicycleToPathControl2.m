...
%20 : coupe un peu, controle tres souple, 100 : coupe un peu, 1000 : triche !
vmax=2.0;
...
%% sliding of horizon if close but add point for next goal
  err = dist(Path(:,goalWaypointId), xtemp);
  distt = norm(err(1:2));
  if (distt<rho)
    ...
    list_points=[list_points,xtemp]; %next point in direction of path
  end
...
%% Then perform P control
    ...
    u(1) = Krho * goalDist/(window_size*10); %vitesse bornee
